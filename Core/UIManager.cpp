...
namespace DuiLib {
		...
		case WM_PAINT:
			{
				...
				if( m_bUpdateNeeded ) {
					...
					if( !::IsRectEmpty(&rcClient) && !::IsIconic(m_hWndPaint) ) {
						...
						if( m_bFirstLayout ) {
							...
							SendNotify(m_pRoot, DUI_WMSGTYPE_WINDOWINIT,  0, 0, false);
				...
				// 发送窗口大小改变消息
				if(bNeedSizeMsg) {
					this->SendNotify(m_pRoot, DUI_WMSGTYPE_WINDOWSIZE, 0, 0, true);
				}
				return true;
			}
		case WM_PRINTCLIENT:
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
		...
		if( m_pFocus != NULL ) 
		{
			...
			event.dwTimestamp = ::GetTickCount();
			m_pFocus->Event(event);
			SendNotify(m_pFocus, DUI_WMSGTYPE_KILLFOCUS);
			m_pFocus = NULL;
		}
			...
			&& pControl->IsEnabled() ) 
		{
			...
			event.dwTimestamp = ::GetTickCount();
			m_pFocus->Event(event);
			SendNotify(m_pFocus, DUI_WMSGTYPE_SETFOCUS);
		}
	}
	...
	void CPaintManagerUI::SendNotify(CControlUI * pControl, WORD wMessage, WPARAM wParam, LPARAM lParam, bool bAsync)
	{
		TNotifyUI Msg;
		Msg.pSender = pControl;
		Msg.wType = wMessage;
	...
	void CPaintManagerUI::SendNotify(TNotifyUI& Msg, bool bAsync /*= false*/)
	{
		...
		{
			Msg.sVirtualWndNameHash = BKDRHash((LPTSTR) Msg.pSender->GetVirtualWnd().GetData());
		}
