...
static const DUI_MSGMAP_ENTRY* DuiFindMessageEntry(const DUI_MSGMAP_ENTRY* lpEntry,TNotifyUI& msg )
{
	...
	{
		if(lpEntry->wMsgType == msg.wType  )
		{
			if(lpEntry->sCtrlNameHash!=0)
			{
				//必要的时候再获取名称字符串
				CDuiString &sCtrlName = msg.pSender->GetName();
				if(lpEntry->sCtrlNameHash ==BKDRHash(sCtrlName))
				{
					return lpEntry;
				}
			}
...
{
	///遍历虚拟窗口
	if( msg.sVirtualWndNameHash!=0){
		for( int i = 0; i< m_VirtualWndMap.GetSize(); i++ ) {
			if( LPCTSTR key = m_VirtualWndMap.GetAt(i) ) {
				unsigned keyhash = BKDRHash(key);
				if(keyhash== msg.sVirtualWndNameHash){
