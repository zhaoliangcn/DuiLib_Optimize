...
namespace DuiLib {
	...
	{
		if (msg.wType== DUI_WMSGTYPE_WINDOWINIT ) {
			EnsureVisible(m_iOldSel);
		}
		else if(msg.wType == DUI_WMSGTYPE_CLICK) {
			// 测试代码
			...
			if( m_pOwner->GetManager() != NULL ) m_pOwner->GetManager()->SendNotify(msg.pSender, DUI_WMSGTYPE_CLICK, 0, 0);
		}
	}
	...
	bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_WMSGTYPE_ITEMSELECT, m_iCurSel, iOldSel);
		Invalidate();
	...
	bool CComboUI::Activate()
	{
		...
		if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_WMSGTYPE_PREDROPDOWN);
		if( m_pWindow ) return true;
		m_pWindow = new CComboWnd();
		ASSERT(m_pWindow);
		m_pWindow->Init(this);
		if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_WMSGTYPE_DROPDOWN);
		Invalidate();
		return true;
	}
