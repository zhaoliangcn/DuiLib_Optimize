...
namespace DuiLib
{
	...
	void CButtonUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_RBUTTONUP)
		{
			if (::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled()) {
				m_uButtonState |= UISTATE_HOT;
				Invalidate();
				m_pManager->SendNotify(this, DUI_WMSGTYPE_RCLICK);	
			}
			return;
		}
		...
		if (event.Type == UIEVENT_MBUTTONUP)
		{
			if (::PtInRect(&m_rcItem, event.ptMouse) && IsEnabled()) {
				m_uButtonState |= UISTATE_HOT;
				Invalidate();
				//m_pManager->SendNotify(this, DUI_MSGTYPE_MCLICK);
				m_pManager->SendNotify(this, DUI_WMSGTYPE_MCLICK);
	...
	bool CButtonUI::Activate()
	{
		...
		if( m_pManager != NULL )
		{
			...
			m_pManager->SendNotify(this, DUI_WMSGTYPE_CLICK);
			BindTriggerTabSel();
		}
		return true;
	}
