...
namespace DuiLib {
	...
	{	
		...
		//复选框
		if(msg.wType == DUI_WMSGTYPE_LISTHEADITEMCHECKED)
		{
		...
		else if (msg.wType== DUI_WMSGTYPE_LISTITEMCHECKED)
		{
			for (int i = 0; i < GetCount(); ++i) {
				CControlUI* p = GetItemAt(i);
				CListTextExtElementUI* pLItem = static_cast<CListTextExtElementUI*>(p->GetInterface(_T("ListTextExElement")));
		...
		{
			if(msg.wType== DUI_WMSGTYPE_SETFOCUS)
			{

			}
			else if(msg.wType == DUI_WMSGTYPE_KILLFOCUS)
			{
		...
		else if (_tcsicmp(strName, _T("ListEx_Combo")) == 0 && m_pComboBoxUI && m_nRow >= 0 && m_nColum >= 0)
		{
			...
			else if(msg.wType==DUI_WMSGTYPE_LISTITEMSELECT && iOldSel >= 0)
			{
				CListTextExtElementUI* pRowCtrl = (CListTextExtElementUI*)GetItemAt(m_nRow);
				if (pRowCtrl)
				{
		...
		else if(msg.wType== DUI_WMSGTYPE_SCROLL && (m_pComboBoxUI || m_pEditUI) && m_nRow >= 0 && m_nColum >= 0)
		{
	...
	void CListContainerHeaderItemUI::DoEvent(TEventUI& event)
	{
			...
			else if( event.Type == UIEVENT_BUTTONUP )
			{
				if( (m_uCheckBoxState & UISTATE_CAPTURED) != 0 )
				{
					if( ::PtInRect(&rcCheckBox, event.ptMouse) ) 
					{
						...
						if (pOwner)
						{
							m_pManager->SendNotify(this, DUI_WMSGTYPE_LISTHEADITEMCHECKED, pOwner->GetItemIndex(this), m_bChecked);
						}
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
				...
				m_pManager->SendNotify(this, DUI_WMSGTYPE_LISTHEADERCLICK);
				Invalidate();
			}
			return;
		}
...
Label_ForeImage:
	...
	void CListTextExtElementUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONUP && IsEnabled() ) {
			for( int i = 0; i < m_nLinks; i++ ) {
				if( ::PtInRect(&m_rcLinks[i], event.ptMouse) ) {
					m_pManager->SendNotify(this, DUI_WMSGTYPE_LINK, i);
					return;
				}
			}
		}
				...
				else if( event.Type == UIEVENT_BUTTONUP )
				{
					if( (m_uCheckBoxState & UISTATE_CAPTURED) != 0 )
					{
						if( ::PtInRect(&rcCheckBox, event.ptMouse) ) 
						{
							...
							if (m_pManager)
							{
								m_pManager->SendNotify(this, DUI_WMSGTYPE_LISTITEMCHECKED, MAKEWPARAM(GetIndex(), 0), m_bChecked);
							}
						}
						m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
						Invalidate();
