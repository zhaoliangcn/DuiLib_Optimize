...
namespace DuiLib {
	...
	void CListUI::DoEvent(TEventUI& event)
	{
		...
		//{
		//	if (IsEnabled()) {
		//		m_pManager->SendNotify(this, DUI_WMSGTYPE_ITEMRCLICK);
		//	}
		//	return;
		//}
	...
	bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
	{
		...
		if( m_pManager != NULL && iLastSel != m_iCurSel) {
			m_pManager->SendNotify(this, DUI_WMSGTYPE_ITEMSELECT, iIndex);
		}
	...
	void CListHeaderItemUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
		{
			...
			if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
				...
				m_pManager->SendNotify(this, DUI_WMSGTYPE_HEADERCLICK);
				Invalidate();
			}
			return;
		}
	...
	bool CListElementUI::Activate()
	{
		...
		if( m_pManager != NULL ) m_pManager->SendNotify(this, DUI_WMSGTYPE_ITEMACTIVATE);
		return true;
	}
	...
	void CListLabelElementUI::DoEvent(TEventUI& event)
	{
		...
		{
			if( IsEnabled() ){
				m_pManager->SendNotify(this, DUI_WMSGTYPE_ITEMCLICK);
			}
			return;
		}
		//if (event.Type == UIEVENT_RBUTTONUP)
	...
	void CListContainerElementUI::DoEvent(TEventUI& event)
	{
		...
		{
			m_pManager->SendNotify(this, DUI_WMSGTYPE_TIMER, event.wParam, event.lParam);
			return;
		}
		...
		if( event.Type == UIEVENT_CONTEXTMENU )
		{
			if( IsContextMenuUsed() ) {
				m_pManager->SendNotify(this, DUI_WMSGTYPE_MENU, event.wParam, event.lParam);
				return;
			}
		}
		// An important twist: The list-item will send the event not to its immediate
